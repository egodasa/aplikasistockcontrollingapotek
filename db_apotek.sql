...
CREATE TRIGGER `tambah_stok_beli` AFTER INSERT ON `tbl_detail_pembelian` FOR EACH ROW update tbl_obat set stok = stok + NEW.jumlah where id_obat = NEW.id_obat;

CREATE TRIGGER `kurang_stok_beli` AFTER DELETE ON `tbl_detail_pembelian` FOR EACH ROW update tbl_obat set stok = stok - OLD.jumlah where id_obat = OLD.id_obat;
...
CREATE TRIGGER `kurangi_stok` AFTER INSERT ON `tbl_detail_transaksi` FOR EACH ROW update tbl_obat set stok = stok - new.jumlah where id_obat = new.id_obat;

CREATE TRIGGER `reset_stok` AFTER DELETE ON `tbl_detail_transaksi` FOR EACH ROW update tbl_obat set stok = stok + OLD.jumlah where id_obat = OLD.id_obat;
...
CREATE VIEW `laporan_detail_transaksi` AS select `a`.`id_transaksi` AS `Id_Transaksi`,`a`.`id_obat` AS `Id_Obat`,`b`.`Nama_Obat` AS `Nama_Obat`,`b`.`Harga_Obat` AS `Harga_Obat`,`a`.`jumlah` AS `Jumlah`,(`a`.`jumlah` * `b`.`Harga_Obat`) AS `Total`,`b`.`Satuan` AS `Satuan` from (`tbl_detail_transaksi` `a` join `daftar_obat` `b` on((`a`.`id_obat` = `b`.`Id_Obat`)));
...
CREATE VIEW `daftar_transaksi` AS select `b`.`Id_Transaksi` AS `Id_Transaksi`,date_format(`a`.`tgl_transaksi`,'%d-%M-%Y') AS `Tanggal_Transaksi`,sum(`b`.`Jumlah`) AS `Jumlah`,sum(`b`.`Total_Harga`) AS `Total_Harga` from (`tbl_transaksi` `a` join `daftar_obat_beli` `b` on((`a`.`id_transaksi` = `b`.`Id_Transaksi`))) group by `a`.`id_transaksi` order by `a`.`tgl_transaksi` desc;

CREATE VIEW `laporan_pemasukan` AS select `a`.`tgl_transaksi` AS `tgl_transaksi`,`b`.`Id_Transaksi` AS `Id_Transaksi`,`b`.`Nama_Obat` AS `Nama_Obat`,`b`.`Jumlah` AS `Jumlah`,`b`.`Total_Harga` AS `Total_Harga` from (`daftar_obat_beli` `b` join `tbl_transaksi` `a` on((`a`.`id_transaksi` = `b`.`Id_Transaksi`)));

CREATE VIEW `lap_harian` AS select `laporan_pemasukan`.`tgl_transaksi` AS `tgl_transaksi`,`laporan_pemasukan`.`Nama_Obat` AS `Nama_Obat`,sum(`laporan_pemasukan`.`Jumlah`) AS `Jumlah`,sum(`laporan_pemasukan`.`Total_Harga`) AS `Total_Harga` from `laporan_pemasukan` where date(`laporan_pemasukan`.`tgl_transaksi`) = date(now()) group by `laporan_pemasukan`.`Nama_Obat`;
...
CREATE VIEW `lap_tahunan` AS select month(`laporan_pemasukan`.`tgl_transaksi`) AS `bulan`,sum(`laporan_pemasukan`.`Jumlah`) AS `Jumlah`,sum(`laporan_pemasukan`.`Total_Harga`) AS `Total_Harga` from `laporan_pemasukan` where (year(`laporan_pemasukan`.`tgl_transaksi`) = year(now())) group by month(`laporan_pemasukan`.`tgl_transaksi`);
